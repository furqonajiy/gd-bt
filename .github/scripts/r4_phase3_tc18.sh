#!/usr/bin/env bash
# Phase-3 TC18 cell: risk-ladder at 1/2/3/5% for ONE Phase-2 top-10 cell.
# Jan-Jun 2026 tick-hybrid (real ticks May-Jun, M1 Jan-Apr); OOS Jun16-26 tick.
# Feed feed.txt must exist (generated Jan-Jun in the workflow with Phase-1 generator params).
# Emits out/{CELL}_r{01,02,03,05}.json (one per risk level).
# Env: JAN_START OOS_TICK CELL ENTRIES slm to tc ad mh
set -o pipefail
mkdir -p out raw
SIGNALS="feed.txt"
...
base = {
    "cell": cell, "strategy": "TC18", "entries": int(entries),
