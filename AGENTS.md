...
it stays 1-entry until ~$10K, then switch to the full TSL18 book.
  **Run ONE book per small account — do NOT pool TS3K with a Victor book
  (V072/V116/V817)** on the same $3k (shared-concurrency starvation; combined DD
...
- **`positions.json`** is the tracked-signal registry (`SignalRegistry` in
  ...
  **`--strategy-tag`** (e.g. `VIC` vs `SC24`) — it is stamped onto `signal_key`
  ...
  parity holds); each executor still needs its own `--positions-json`.
  ...
  **a trailing-open strategy never rests a LIMIT entry** — every placement and every
  ...
  On a **partial FOLLOW** (the replay filtered out legs it marks already-played-out)
  `--trailing-live-entry` **rebuilds the FULL ladder** (`_restore_trailing_ladder_orders`):
  every dropped leg is restored from its `compute_entries` PLANNED price (SL/lot from
  the replay leg) and placed as a trailing-open STOP, so a fast signal never silently
