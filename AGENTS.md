...
- **Small-account deployment-safety gates** (`strategy/deployment_gate.py`,
  ...
  **limited 2-entry from $2K, switch to full 8-entry ~$10K** (stop → change
  ...
  analogue of TS2K. **TS3K** (`cli/candidate_TS3K_small_account_tick.txt`, tag
  **TS3K**, *research/demo*) is the **$3k** sibling of TS2K — identical T818/TSL18
  feed+geometry and the SAME equity-relative 2-entry gates (entries 2 / max-open 1 /
  daily 5% / risk-budget zone 6%·single 4%), only `--initial-capital 3000`. $3k is
  STILL under the full-8-entry floor (~$4k p95 → ~$8k worst stop), so it stays
  2-entry (the extra $1k vs TS2K is pure DD cushion, not more legs) until ~$10K.
