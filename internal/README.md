...
| `auto`     | Continuous live trading: read `signals.txt`, place orders, manage positions, append-only event log. `--replace-missing-entries` self-heals pending entries cancelled by hand; `--reopen-missing-positions` restores positions closed by hand while the replay still holds them OPEN **and** places partially played-out signals per entry instead of skipping them. Both self-heals follow the strategy: with `trailing_open_distance = 0` they re-place a price-aware LIMIT/market entry (market only at-or-better than the entry, else a LIMIT at the original entry — never chases); with `trailing_open_distance > 0` they **always re-arm the trailing-open STOP** (a trailing strategy never rests a LIMIT). The trailing re-entry gate is **reason-aware**: an SL/TP/stop-out/engine close counts as "already traded" (no re-entry), but a manual hand-close (deal reason CLIENT/MOBILE/WEB) re-arms the trailing-open. `--adaptive` auto-switches by **volatility regime**: each cycle it classifies the current market and runs that regime's published champion config (`CHAMPION_<regime>.json`), falling back to the incumbent when none exists. |
