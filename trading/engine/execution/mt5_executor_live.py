"""Live-parity MT5 executor wrapper.
...
``mt5_executor_trailing.py`` (the one re-exported as ``Mt5Executor``). Its job is
the live-only wall-clock / activation / expiry parity guards listed below (the
TP2 stop-lock is just one of them) -- hence ``_live``.
