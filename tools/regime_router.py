"""Live regime detection for the adaptive auto-router.
...
so the router and the signal generator stay consistent. Boundaries are placed
between adjacent regime ATR levels.

This module is intentionally dependency-light (pandas only) so it can run inside
the live `auto` loop. Champion selection (reading CHAMPION_<regime>.json and
comparing to the incumbent) is wired in once the grid's deploy format is pinned
by a real checkpoint.
"""
