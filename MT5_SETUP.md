...
to **per-month CSV files** in `data/` (override with `--archive-dir`). Over
time you accumulate a growing archive that goes beyond what MT5 itself
keeps — exactly the data you need for re-running backtests on your own
broker's quotes.
...
If you want strict overwrite (replace the file with only what this fetch
returned) pass `--archive-overwrite` — be aware this can erase older data
on boundary months as MT5's window shifts.

### Archive flags

| Flag | Default | Meaning |
|---|---|---|
| `--archive-dir DIR` | `data/` | Where to write the per-month files |
| `--archive-months N` | `4` | How many calendar months back to fetch |
| `--archive-overwrite` | off | Strict overwrite instead of merge |
| `--no-archive` | off | Skip archiving (decide only, no save) |
...
```powershell
python -m xauusd_trading.cli fetch --mt5-symbol XAUUSD --archive-months 6
```

Useful for filling the archive on a schedule (e.g. a daily Task Scheduler
job that runs `xauusd fetch` to capture the latest day before older data
falls off MT5's window).

### Using the archive for backtests

Once you have monthly files, run backtests against your own broker's data:
...
The format is identical to the original CSVs in this repo, so they're
...
- **Auto-detect open positions from MT5.** MT5 stores ticket / open price /
  ...
  the signal text. That requires either consistent comment-tagging plus a
  local signal catalog, or a richer order-management system. Out of scope
  for the current iteration; stick with `positions.json` for now.
- **Auto-place orders.** `decide` prints what to place; you place them. This
  is deliberate — the engine is read-only on your account. Auto-execution is
  a separate, bigger system.
